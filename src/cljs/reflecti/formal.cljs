...
(defn input
  ...
  :container        - React   - A react component that can be provided as a
                                container to this input.
...
(defmethod spec->input :default
  ...
  [div [span (str "Unknown spec: " spec-id " " [(:ui props) spec-key])]])
...
(defmethod spec->input :clojure.spec.alpha/keys
  ...
  (let [spec-form-args (->> (s/form spec) rest (partition 2 2))
    ...
    (fn [{::keys [spec spec-id children form-state path ui] :as props}]
      (let [this (r/current-component)
        ...
        [div
         ...
         [div {:style (:inner-container (style props))}
          (into
           [component
